 ...
 * mime_parser.php
 *
 * @(#) $Id: mime_parser.php,v 1.44 2008/01/22 04:46:11 mlemos Exp $
 *
 */
...
<class>
	...
	<version>@(#) $Id: mime_parser.php,v 1.44 2008/01/22 04:46:11 mlemos Exp $</version>
...
class MimeComponent extends Object {
	var $components = array('Rfc822');
/*
{metadocument}
	<variable>
		<name>error</name>
	...
	<variable>
		...
		<documentation>
			...
			<usage>Change this variable if you need to extract e-mail addresses
				...
				<stringvalue>cc:</stringvalue> and
				<stringvalue>bcc:</stringvalue>.</usage>
		</documentation>
	</variable>
{/metadocument}
*/
							...
							case MIME_PARSER_BODY_START:
								for($position = $this->body_buffer_position; ;)
								{
									...
									if(!strcmp(substr($this->body_buffer, $position, $line_break - $position), $boundary))
									{
										...
										$this->body_parser = new MimeComponent;
		...
		<argument>
			...
			<documentation>
				<purpose>Returns an associative array with the results of the
					...
					types: <tt>binary</tt>, <tt>text</tt>, <tt>html</tt>, <tt></tt>,
					...
					Character set encoding of the message.<paragraphbreak />
...
*/
	Function Analyze($message, &$results)
	{
		...
		if(!IsSet($results['Type']))
			return($this->SetErrorWithContact('Analyzing messages with parts of type '.$content_type.' it is not yet supported'));
